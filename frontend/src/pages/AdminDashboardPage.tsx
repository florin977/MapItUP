...
interface AdminDashboardPageProps {
  ...
  /** vine din App.tsx: () => setCurrentPage(Page.ChooseFloor) */
  goToChooseFloor: () => void;
}
...
const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({
  ...
  // timeout pt feedback upload
  useEffect(() => {
    if (!uploadFeedback) return;
    const id = setTimeout(() => setUploadFeedback(null), 5000);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === "image/svg+xml") {
      ...
      reader.onload = (e) => {
        ...
        setUploadFeedback({
          message: `SVG for ${
            uploadingFloor.charAt(0).toUpperCase() + uploadingFloor.slice(1)
          } Floor uploaded successfully!`,
          type: "success",
        });
      };
      reader.onerror = () => {
      ...
      setUploadFeedback({
        message: "Please select a valid SVG file.",
        type: "error",
      });
    }
      ...
      case "campus":
        return (
          <div className="space-y-6 animate-fadeIn">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-brand-text-primary tracking-tight">
              Manage Campus Map (2D)
            </h2>

            {/* input pentru upload SVG */}
            <input
              type="file"
              ref={fileInputRef}
            ...
            {uploadFeedback && (
              <div
                className={`p-4 mb-2 text-sm rounded-lg transition-opacity duration-300 ${
                  uploadFeedback.type === "success"
                    ? "bg-green-100 dark:bg-green-800/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-600/50"
                    : "bg-red-100 dark:bg-red-800/30 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-600/50"
                }`}
                role="alert"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Card 1: upload direct din Admin (selectezi etajul aici) */}
              <div className="space-y-3">
                <AdminActionCard
                  icon={
                    <UploadIcon className="w-8 h-8 text-violet-500 dark:text-brand-primary" />
                  }
                  title="Upload 2D Layout"
                  description="Upload a custom 2D SVG layout for a specific floor."
                  actionText="Upload SVG now"
                  onActionClick={() => setActiveSection("campus")}
                />
                <div className="grid grid-cols-3 gap-2">
                  {(["ground", "first", "second"] as const).map((floor) => (
                    <button
                      key={floor}
                      onClick={() => handleFileUploadClick(floor)}
                      className="px-2 py-1 text-xs font-semibold rounded-md bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-brand-text-primary hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    >
                      {floor === "ground"
                        ? "Ground"
                        : floor === "first"
              ...
              {/* Card 2: butonul care deschide altă pagină (ChooseFloorPage) */}
              <AdminActionCard
                icon={
                  <MapIcon className="w-8 h-8 text-violet-500 dark:text-brand-primary" />
                }
                title="Choose Floor Page"
                description="Go to a dedicated page where you can choose a floor and configure uploads."
                actionText="Go to Choose Floor"
                onActionClick={goToChooseFloor}
              />
            </div>
            ...
            <div className="p-6 bg-white dark:bg-brand-surface rounded-lg shadow-sm border border-slate-200 dark:border-slate-800">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-slate-800 dark:text-brand-text-primary">
                  Current Map Preview:
                </h3>
                <div className="bg-slate-100 dark:bg-slate-800/50 p-1 rounded-lg flex items-center space-x-1">
                  {(["ground", "first", "second"] as const).map((floor) => {
                    const floorLabel =
                      floor === "ground" ? "G" : floor === "first" ? "1" : "2";
                    return (
                      <button
                        key={floor}
                        ...
                        className={`px-3 py-1 text-sm font-semibold rounded-md transition-all duration-200 w-9 ${
                          previewFloor === floor
                            ? "bg-violet-600 dark:bg-brand-primary text-white shadow"
                            : "text-slate-600 dark:text-brand-text-secondary hover:bg-black/5 dark:hover:bg-slate-700/50"
                        }`}
                        title={`${
    ...
    <div className="flex h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-violet-100 dark:bg-brand-background">
      <aside className="w-64 bg-white/95 dark:bg-brand-surface border-r-2 border-violet-200 dark:border-slate-800 flex flex-col shadow-2xl shadow-indigo-500/10 dark:shadow-none">
        <div className="p-6 border-b-2 border-violet-200 dark:border-slate-800">
          <h1 className="text-xl font-extrabold bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 dark:text-brand-primary bg-clip-text text-transparent tracking-tight">
            Admin Panel
          </h1>
          <p className="text-sm font-bold text-indigo-700 dark:text-brand-text-secondary">
            MapItUP
          </p>
        </div>
      ...
      {isAddModalOpen && (
        <AddBuildingModal onClose={handleCloseModals} onAdd={onAddBuilding} />
      )}
...
const DashboardSection = ({
  ...
  const maxSearches =
    popularRoutes.length > 0 ? Math.max(...popularRoutes.map((r) => r.count)) : 1;
    ...
    <div className="animate-fadeIn space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-indigo-700 dark:text-slate-400 font-semibold">
          ...
          <div className="space-y-5">
            {popularRoutes.length > 0 ? (
              popularRoutes.map((routeData, index) => (
                <div
                  key={routeData.route}
                  className="group animate-fadeInUp hover:bg-indigo-50/50 dark:hover:bg-slate-800/30 p-4 rounded-2xl transition-all duration-300"
                ...
                >
                  <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <span
                        className="font-bold text-indigo-900 dark:text-brand-text-secondary truncate pr-4"
                        title={routeData.route}
                      >
                        {routeData.route}
                      </span>
                    </div>
                    <div className="px-3 py-1.5 bg-violet-100 dark:bg-violet-900/30 rounded-lg">
                      <span className="font-extrabold text-violet-700 dark:text-brand-text-primary text-sm">
                        {routeData.count}{" "}
                        <span className="text-xs font-semibold text-violet-600 dark:text-slate-500">
                          searches
                        </span>
                      </span>
                    </div>
                  ...
                  <div className="w-full bg-gradient-to-r from-violet-100 to-purple-100 dark:bg-slate-800 rounded-full h-3 shadow-inner overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(routeData.count / maxSearches) * 100}%`,
                      }}
                    ></div>
                  </div>
                </div>
              ...
              <div className="w-full h-64 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-indigo-700 dark:text-brand-text-secondary font-semibold">
                    No route data yet.
                  </p>
                  <p className="text-sm text-slate-500 dark:text-slate-500 mt-2">
                    Perform searches in guest view to see popular routes!
                  </p>
                </div>
              </div>
            )}
          ...
          <ul className="space-y-4">
            {uploadActivities.length > 0 ? (
              uploadActivities.map((activity, index) => (
                <li
                  key={index}
                  className="flex items-start space-x-3 p-3 rounded-xl hover:bg-indigo-50/50 dark:hover:bg-slate-800/30 transition-all duration-200"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-indigo-700 dark:text-slate-400 mt-1">
                      <span className="font-semibold text-violet-600 dark:text-brand-text-primary">
                        {activity.floor.charAt(0).toUpperCase() +
                          activity.floor.slice(1)}{" "}
                        floor
                      </span>
                    </p>
              ...
              <li className="text-center py-12">
                ...
                <p className="text-sm font-semibold text-indigo-700 dark:text-brand-text-secondary">
                  No recent activity
                </p>
      ...
      <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400">
        ...
        <tbody>
          {buildings.map((building) => (
            <tr
              key={building.id}
              className="bg-white dark:bg-brand-surface border-b dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50"
            >
              <th
                scope="row"
              ...
              <td className="px-6 py-4 text-right space-x-2">
                <button
                  onClick={() => onEditBuildingClick(building)}
                  className="font-medium text-teal-600 dark:text-brand-accent hover:underline p-1"
                  aria-label={`Edit timetable for ${building.name}`}
                >
...
const AnalyticsSection = () => (
  <div className="animate-fadeIn space-y-8">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-white/95 dark:bg-brand-surface p-8 rounded-3xl shadow-2xl shadow-indigo-500/20 dark:shadow-none border-2 border-violet-200 dark:border-slate-800 hover:shadow-3xl hover:shadow-indigo-500/30 transition-all duration-300">
        <h3 className="text-xl font-extrabold bg-gradient-to-r from-violet-600 to-purple-600 bg-clip-text text-transparent dark:bg-none dark:text-white mb-6">
          Peak Usage Hours
        </h3>
        <div className="h-64 flex items-end justify-around p-4 space-x-3">
          {["9am", "11am", "1pm", "3pm", "5pm", "7pm"].map((label, idx) => {
            const heights = [30, 60, 95, 80, 50, 20];
            return (
              <div key={label} className="group flex flex-col items-center flex-1">
                <div
                  className={`w-full rounded-t-2xl shadow-lg transition-all duration-300 ${
                    idx === 2
                      ? "bg-gradient-to-t from-fuchsia-500 to-pink-600"
                      : "bg-gradient-to-t from-violet-500 to-purple-600"
                  }`}
                  style={{ height: `${heights[idx]}%` }}
                ></div>
                <span className="text-xs font-bold text-indigo-700 dark:text-slate-400 mt-2">
                  {label}
                </span>
              </div>
            );
        ...
        <div className="h-64 flex items-center justify-center">
          <div className="relative">
            <div
              ...
              style={{
                background:
                  "conic-gradient(#8b5cf6 0% 65%, #2dd4bf 65% 90%, #c4b5fd 90% 100%)",
              }}
            >
              <div className="w-full h-full flex items-center justify-center">
                <div className="w-28 h-28 bg-white dark:bg-brand-surface rounded-full shadow-inner flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xs font-bold text-indigo-700 dark:text-slate-400">
                      Active
                    </p>
                  </div>
                </div>
  ...
  return (
    <div className="group bg-white/95 dark:bg-brand-surface p-6 rounded-3xl shadow-lg shadow-indigo-200/50 dark:shadow-none hover:shadow-2xl hover:shadow-indigo-400/50 dark:hover:shadow-[0_0_20px_rgba(167,139,250,0.2)] transition-all duration-300 border-2 border-violet-200 dark:border-slate-800 hover:border-violet-400 dark:hover:border-violet-700 transform hover:-translate-y-2">
      <div className="flex items-center justify-between mb-4">
        <div
          className={`p-4 ${iconBg} rounded-2xl shadow-xl transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}
        >
          {icon}
        </div>
      </div>
      <div className="space-y-2">
        <p className="text-sm font-bold text-indigo-700 dark:text-brand-text-secondary uppercase tracking-wide">
          {title}
        </p>
        <p className="text-4xl font-black bg-gradient-to-r from-violet-600 to-purple-600 dark:text-brand-text-primary bg-clip-text text-transparent">
          {value}
        </p>
...
const AdminActionCard: React.FC<AdminActionCardProps> = ({
  ...
  <div className="bg-white/95 dark:bg-brand-surface p-6 rounded-2xl shadow-lg shadow-indigo-200/50 dark:shadow-none hover:shadow-2xl hover:shadow-indigo-300/50 dark:hover:shadow-[0_0_15px_rgba(167,139,250,0.2)] transition-all duration-300 flex flex-col border-2 border-violet-200 dark:border-slate-800 hover:border-violet-400 dark:hover:border-brand-primary/50 transform hover:-translate-y-1">
    <div className="flex items-center mb-4">
      {icon}
      <h3 className="ml-4 text-lg font-extrabold text-indigo-900 dark:text-brand-text-primary">
        {title}
      </h3>
    </div>
    <p className="text-indigo-700 dark:text-brand-text-secondary text-sm font-medium flex-grow">
      {description}
    </p>
    <button
...
const AddBuildingModal: React.FC<{
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center animate-fadeIn"
      onClick={onClose}
    >
      <div
        className="bg-white dark:bg-brand-surface rounded-lg shadow-2xl w-full max-w-md p-8 m-4 animate-fadeInUp"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-2xl font-bold text-slate-900 dark:text-brand-text-primary mb-6">
          Add New Building
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label
              ...
              className="block text-sm font-medium text-slate-700 dark:text-brand-text-secondary"
            >
              Building Name
            </label>
            <input
              ...
              required
              className="mt-1 block w-full pl-3 py-2 border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-brand-text-primary rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-primary"
            />
          </div>
          <div className="flex justify-end pt-4 space-x-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-brand-text-secondary bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-md transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-2 text-sm font-medium text-white bg-brand-primary hover:bg-brand-secondary rounded-md transition-colors"
            >
              Add Building
            </button>
          </div>
...
const TimetableModal: React.FC<{
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center animate-fadeIn p-4"
      onClick={onClose}
    >
      <div
        className="bg-white dark:bg-brand-surface rounded-lg shadow-2xl w-full max-w-5xl animate-fadeInUp flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-brand-text-primary">
            Edit Timetable: {building.name}
          </h2>
          <p className="text-sm text-slate-500 dark:text-brand-text-secondary">
        ...
        <div className="overflow-auto px-6 pb-6">
          <table className="w-full text-sm text-left border-separate border-spacing-x-1 border-spacing-y-1">
            <thead className="sticky top-0 z-10">
              <tr>
                <th className="p-2 font-semibold text-slate-600 dark:text-brand-text-secondary w-24 bg-white dark:bg-brand-surface">
                  Time
                </th>
                {days.map((day) => (
                  <th
                    key={day}
                    className="p-2 font-semibold text-slate-600 dark:text-brand-text-secondary capitalize bg-white dark:bg-brand-surface text-center"
                  >
                    {day}
                  </th>
                ))}
            ...
            <tbody>
              {hours.map((hour, hourIndex) => (
                <tr key={hour}>
                  <td className="py-2 px-3 font-mono text-slate-500 dark:text-brand-text-secondary whitespace-nowrap">
                    {hour}
                  </td>
                  {days.map((day) => {
                    ...
                    const isAvailable =
                      !currentValue ||
                      currentValue.toLowerCase() === "available";
                    return (
        ...
        <div className="flex justify-end p-6">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-brand-text-secondary bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-md transition-colors mr-3"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={handleSave}
            className="px-6 py-2 text-sm font-medium text-white bg-brand-primary hover:bg-brand-secondary rounded-md transition-colors"
          >
            Save Changes
          </button>
        </div>
