...
def to_grid_idx(x, y):
    """World → Grid"""
    j = int((x - x_min) / cell_size)
    i = int((y - y_min) / cell_size)
    return i, j

def grid_to_world(i, j):
    """Grid → World"""
    x = x_min + (j + 0.5) * cell_size
    y = y_min + (i + 0.5) * cell_size
    z = floor_z_max
    return x, y, z
...
transform_data = {
    ...
    "note": "To convert grid (i,j) to world: x = x_min + (j+0.5)*cell_size, y = y_min + (i+0.5)*cell_size, z = z_floor"
}
